...
| 永磁同步电机 |                                                                          表贴式电机,参数为额定电压12V,最大电流1A,Rs=2.75Ohm,Ls=1mH,flux=0.00386335(V·s)                                                                           |
...
我们在"Simulink"文件夹里放置了我们的仿真模型，利用该仿真模型，可以快速观察到各变量的变化情况，也可以在无实物的情况下验证算法的可行性，为了验证代码的准确性，该仿真模型大部分使用S-Function Builder构建。以下是一些仿真的结果图片：
...
![磁链观测器观测值与实际值对比](images/Simulink/磁链观测器观测值与实际值对比.png)

EKF滤波前后对比:

![EKF滤波前后对比](images/Simulink/EKF滤波前后对比.png)

SMO与STSMO（待调整）估计反电动势与实际反电动势对比:

![SMO与STSMO（待调整）估计反电动势与实际反电动势对比](images/Simulink/SMO与STSMO（待调整）估计反电动势与实际反电动势对比.png)

不同观测器速度估计值与实际值的阶跃响应对比:

![不同观测器速度估计值与实际值的阶跃响应对比](images/Simulink/不同观测器速度估计值与实际值的阶跃响应对比.png)

不同观测器估计角度与实际角度对比:

![不同观测器估计角度与实际角度对比](images/Simulink/不同观测器估计角度与实际角度对比.png)


## 📎 **补充内容**

·在使用仿真模型时，注意不要将运行模式切换到"加速"或"快速加速"，而是使用"普通"，因为S-Function Builder在"加速"或"快速加速"似乎有些问题，仿真一次后，S-Function Builder里面的变量会维持仿真结束的值，如果再次仿真，建议重新编译一次代码或关闭MATLAB重新打开仿真文件，否则会出现一开始仿真，EKF的预测转速就是几百rad/s的奇怪结果(包括PI控制器保留原来的输出的问题)
...
·仿真模型的S-function-builder在打开时，会优先打开缓存在磁盘里的.c/.h文件，因此当此项目的.slx文件更新时，你打开的新的仿真模型，点开S-function-builder看到的可能只是上一版仿真模型缓存的代码(旧代码)，请手动清理缓存文件，再打开仿真模型，这样才能看到新的代码

缓存清理方法：输入which -all Speed_PID(任意一个S-function-builder的名字都可以)
![matlab缓存查找](images/matlab缓存查找.png)

得到的路径显示缓存文件在桌面上，我们删除对应的缓存文件，重新打开MATLAB，即可看到最新的代码:
![被放在桌面的缓存文件](images/被放在桌面的缓存文件.png)
