...
| 永磁同步电机 | 型号:2804云台电机,表贴式电机,参数为额定电压12V,最大电流1A,相电阻Rs=2.75Ohm,相电感Ls=1mH,磁通flux=0.00386335(V/(rad/s)),<br>如果使用本项目的评估板,建议将电阻设置为3.75(要考虑MOSFET的导通电阻和采样电阻,本评估板导通电阻和采样电阻比较大,需折合到电机侧进行补偿),电感可以增加到1.6mH(根据离线参数辨识得到,离线参数辨识将在V2版本发布,V1暂不考虑发布) |
...
6.考虑到每个硬件版本不同，需要进行非线性补偿，包括死区补偿、导通电阻以及反向导通压降等，请根据实际情况适当增加电机的相电阻Rs以及修改DeadTimeComp进行补偿(实测补偿后12V和500rad/s给定下可以跑到5W功率，但是未补偿时只能跑到4W)
